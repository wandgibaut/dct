...
def add_consumer_codelet_to_system(codelet_folder, ip_port_hostmode, source_codelets_ips_and_memories):
    with open(codelet_folder + '/codelet/fields.json', 'r+') as json_data:
        ...
        new_inputs = []
        for i in range(0,len(source_codelets_ips_and_memories),2):
            source_codelet = source_codelets_ips_and_memories[i]
            ip_port = convert(":", source_codelet)
            #print(source_codelets_ips_and_memories)
            print(ip_port)
            source_codelet_info = dct.getCodeletInfo(ip_port[0], ip_port[1])

            for mem in source_codelet_info['outputs']:
                if mem not in codelet_info['inputs'] and mem['name'] == source_codelets_ips_and_memories[i+1]:
...
def add_feeder_codelet_to_system(codelet_folder, ip_port_hostmode, source_codelets_ips_and_memories):
    with open(codelet_folder + '/codelet/fields.json', 'r+') as json_data:
        codelet_info = json.load(json_data)
        ...
        for i in range(0,len(source_codelets_ips_and_memories),2):
            ...
            for mem in source_codelet_info['inputs']:
                if mem not in codelet_info['outputs'] and mem['name'] == source_codelets_ips_and_memories[i+1]:
...
def add_random_consumer(codelet_folder, ip_port_hostmode, number_of_feeders, list_of_codelets_json):
    with open(list_of_codelets_json, 'r+') as json_data:
        ...
        add_consumer_codelet_to_system(codelet_folder, ip_port_hostmode, source_codelets_ips)
    ...
    with open(ip_port_hostmode_list_json, 'r+') as json_list:
        ...
        with open(list_of_codelets_json, 'r+') as json_data:
            ...
            for i in range(int(number_of_codelets)):
                ...
                add_consumer_codelet_to_system(codelet_folder, ip_port_hostmode_list[i],source_codelets)
                #print (i)
...
if __name__ == '__main__':
    ...
    elif args[0] == 'specific-consumer':
        if len(args) > 3:
            add_consumer_codelet_to_system(args[1], args[2], args[3:])
        else:
            print('Error! Wrong number of arguments!')
    elif args[0] == 'specific-feeder':
        if len(args) > 3:
            add_feeder_codelet_to_system(args[1], args[2], args[3:])
        else:
            print('Error! Wrong number of arguments!')
    elif args[0] == 'scale-consumer':
